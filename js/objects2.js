...
function CookieStore(name, min, max, averageSale) {
  ...
  this.render = function() {
    ...
    //this.cookiesSoldEachHour = []; cookies sold each hour are stored in this variable
  }
}
...
for (let i = 0; i < theStores.length; i++) {
  for (let j = 0; j < theStores[i].cookiesSoldEachHour.length; j++) {
     dailyGrandTotal += theStores[i].cookiesSoldEachHour[j];
      // sum += theStores[i].cookiesSoldEachHour[j];
      //console.log(theStores[i].cookiesSoldEachHour[j]);
         
    // console.log(dailyGrandTotal);
    }
  }
  // This loop calculates the store totals per hour
  let totalHourlySalesArr = Array(14).fill(0); // Array that stores each hour

  for (let i = 0; i < theStores.length; i++) {
    for (let j = 0; j < theStores[i].cookiesSoldEachHour.length; j++) {
      totalHourlySalesArr[j] += theStores[i].cookiesSoldEachHour[j];
      //console.log(totalHourlySalesArr);
    }
    // Calculate the daily store totals for each store
    let dailyStoreTotals = Array(5).fill(0);
      for (let i = 0; i < theStores.length; i++) {
        for (let j = 0; j < theStores[i].cookiesSoldEachHour.length; j++) {
          dailyStoreTotals[i] += theStores[i].cookiesSoldEachHour[j];
          console.log(dailyStoreTotals);
        }
      }
  }
}
